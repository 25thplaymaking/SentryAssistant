...
- [x] **4. Fork per-user login + repoint** (`SentryWebUI` `frontir` branch) — **DONE: code-complete + unit-tested** (fork commits `04bc788c`→`f07f9f36`, 23 tests: dialect gate, SSE translation, enrollment login, per-user token store, cookie→worker threading, login-form field, auto-refresh). **e2e still needs a supervised deploy** (`dialect=sentry` + real enrollment codes) — NOT yet deployed/flipped. Fully mapped 2026-07-21; the browser speaks the fork's OWN named-SSE events (`token`/`reasoning`/`done`/…) and `api/gateway_chat.py` translates, so **the repoint is entirely server-side (no JS changes).** Splits into two independent, sequenced sub-parts:
  - **4a — chat-repoint (small/med, server-side, flag-gated, NO user-facing deploy needed to develop):** add a `sentry` gateway dialect (new reader by `gateway_chat.py:127`); in the worker `_run_gateway_chat_streaming` (`gateway_chat.py:650`) add an isolated branch: URL → `{base_url}/api/chat/turn`, body → `{prompt, session_id, quoted_context}` (replacing the `/v1/chat/completions` build at `:842-878`), and translate Sentry SSE (`{type,summary,maySpeak}`) → existing events: `type=="message"` → `put_gateway_event("token",{"text":summary})`, `turn.completed` → done/`stream_end` (+ wire `maySpeak` to TTS), `error`/`turn.failed` → error, `[DONE]` → stop. Keep the Sentry-SSE→events translation a pure, unit-tested function. Default dialect stays `hermes` so current behaviour is untouched. **Risk the map flagged:** the browser accumulates incremental `token` deltas; our route DOES emit incremental `MESSAGE` events during a real turn (the live proof's fake listener only sent one) — map `message`→`token` so typing stays incremental.
  - **4b — per-user identity (med/large; this is the CHECKPOINT — deploys the user-facing app):** login (`static/login.js:60` + handler `routes.py:16292`) gains a user + enrollment-code field; redeem the Gateway enrollment code → access+refresh tokens; stash the token in the session record (schema already supports extra keys — `auth.py:585`, retrieved via `get_session_info` `auth.py:793`); thread the per-user token from `_handle_chat_start` (`routes.py:21912`) into the worker (spawned `routes.py:21160`) so chat uses the user's token, not the shared `_gateway_api_key()`. New enrollment/token-lifecycle module (none exists today). Sequence 4a → 4b (4a proves the Sentry contract with a shared token before identity is added).

**Restructured downstream phases (Bryce, 2026-07-21):** after slice 1 (P1 = multi-tenant chat via Gateway),
